...
class VideoOverlayModule : Module() {
    ...
    AsyncFunction("burnTextOverlay") { inputUri: String, overlaysRaw: List<Map<String, Any>>, promise: Promise ->
      ...
      Thread {
        try {
          ...
          processVideo(srcPath, outFile.absolutePath, overlays)
  ...
  // ---------------------------------------------------------------------------
  // Core pipeline: decode → bitmap → draw overlays → encode → mux
  // ---------------------------------------------------------------------------
  private fun processVideo(srcPath: String, dstPath: String, overlays: List<OverlaySpec>) {
    ...
    // Decode to ARGB bitmap via ImageReader surface trick — use software decoder path
    // Force software decoding by requesting COLOR_FormatYUV420Flexible output
    // Actually use ImageReader surface: decoder → Surface(ImageReader) → Bitmap
    ...
    // Add audio track to muxer eagerly (before muxer.start)
    var muxAudio = -1
    ...
    if (audioTrack >= 0) {
      ...
      muxAudio = muxer.addTrack(extractor.getTrackFormat(audioTrack).also {
        // re-read from a fresh extractor to get audio format
      }.let { audioExt.getTrackFormat(0) })
    } else {
      audioExt = null
    }
